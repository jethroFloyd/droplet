function [p,w,N] = nrbeval_octave(nurbs,tt)
% 
...
%   This function utilises the 'C' interface bspeval_octave.
% 
% Examples:
% 
...
if (iscell(nurbs.knots))
  if (size(nurbs.knots,2) == 3)
    ...
    if (iscell(tt))
      ...
      val = bspeval_octave (degree(1), val, nurbs.knots{1}, tt{1});
      ...
      for v = 1:nt
        ...
        val2(:,v) = bspeval_octave_octave(degree(2),coefs,nurbs.knots{2},tt(2,v));
      end
      ...
      for v = 1:nt
        ...
        pnts(:,v) = bspeval_octave(degree(1),coefs,nurbs.knots{1},tt(1,v));
      end
  ...
  elseif (size(nurbs.knots,2) == 2)
    ...
    if (iscell(tt))
      ...
      [val,basis] = bspeval_octave(degree(2),val,nurbs.knots{2},tt{2});
      val = reshape(val,[4 num1 nt2]);
    
      %% Evaluate along the u direction
      val = permute(val,[1 3 2]);
      val = reshape(val,4*nt2,num1);
      [val,basis] = bspeval_octave(degree(1),val,nurbs.knots{1},tt{1});
  ...
  [val,N] = bspeval_octave(nurbs.order-1,nurbs.coefs,nurbs.knots,tt(:)');
