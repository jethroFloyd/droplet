...
%   usethird : Use third order neighbour Triangulation for the curvature
...
%     p1=FV.Triangulation-2*Dir1; p2=FV.Triangulation+2*Dir1;       
%     plot3([p1(:,1) p2(:,1)]',[p1(:,2) p2(:,2)]',[p1(:,3) p2(:,3)]','g-');
%     axis equal; view(3) 
%   figure, title('Principal B');
%     p1=FV.Triangulation-2*Dir2; p2=FV.Triangulation+2*Dir2;       
...
% Number of Triangulation
nv=size(FV.Triangulation,1);

% Calculate Triangulation normals
N=patchnormals(FV);
...
% Get neighbours of all Triangulation
Ne=vertex_neighbours(FV);

% Loop through all Triangulation
Lambda1=zeros(nv,1);
Lambda2=zeros(nv,1);
Dir1=zeros(nv,3);
Dir2=zeros(nv,3);
...
% of the faces.
[Nx,Ny,Nz]=patchnormals_double(double(FV.faces(:,1)),double(FV.faces(:,2)),double(FV.faces(:,3)),double(FV.Triangulation(:,1)),double(FV.Triangulation(:,2)),double(FV.Triangulation(:,3)));
N=zeros(length(Nx),3);
...
FV.Triangulation=zeros(length(Vx),3);
FV.Triangulation(:,1)=Vx;
FV.Triangulation(:,2)=Vy;
FV.Triangulation(:,3)=Vz;

% Get all edge vectors
e1=FV.Triangulation(Fa,:)-FV.Triangulation(Fb,:);
e2=FV.Triangulation(Fb,:)-FV.Triangulation(Fc,:);
e3=FV.Triangulation(Fc,:)-FV.Triangulation(Fa,:);
...
% Calculate Angle of face seen from Triangulation
...
VerticeNormals=zeros([size(FV.Triangulation,1) 3]);
for i=1:size(Fa,1),
...
% Ne=vertex_neighbours(FV)
%
Ne=vertex_neighbours_double(FV.faces(:,1),FV.faces(:,2),FV.faces(:,3),FV.Triangulation(:,1),FV.Triangulation(:,2),FV.Triangulation(:,3));
...
for i=1:size(V,1)
    ...
    if(isempty(Pneighf))
        ...
        for j=2+double(found):(length(Pneighf)/2)
            ...
            if(~found) % This only happens with weird edge Triangulation
                for index=1:2:length(Pneighf),
